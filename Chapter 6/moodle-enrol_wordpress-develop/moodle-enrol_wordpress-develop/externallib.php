...
/**
 * WordPress enrol plugin external functions
 *
...
/**
 * WordPress enrolment external functions.
 *
 * @package   enrol_wordpress
 * @copyright 2017 Ian Wild
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     Moodle 3.1
 */
class enrol_wordpress_external extends external_api {
    ...
    public static function get_instance_info_parameters() {
        return new external_function_parameters(
                array('instanceid' => new external_value(PARAM_INT, 'instance id of WordPress enrolment plugin.'))
            );
    }

    /**
     * Return wordpress-enrolment instance information.
     *
     * @param int $instanceid instance id of WordPress enrolment plugin.
    ...
    public static function get_instance_info($instanceid) {
        ...
        // Retrieve WordPress enrolment plugin.
        $enrolplugin = enrol_get_plugin('wordpress');
    ...
    public static function enrol_user_parameters() {
        return new external_function_parameters(
            array(
                ...
                'instanceid' => new external_value(PARAM_INT, 'Instance id of WordPress enrolment plugin.', VALUE_DEFAULT, 0)
            )
        );
    }

    /**
     * WordPress enrol the current user in the given course.
     *
    ...
    public static function enrol_user($courseid, $password = '', $instanceid = 0) {
        ...
        // Retrieve the WordPress enrolment plugin.
        $enrol = enrol_get_plugin('wordpress');
        if (empty($enrol)) {
            throw new moodle_exception('canntenrol', 'enrol_wordpress');
        }

        // We can expect multiple wordpress-enrolment instances.
        $instances = array();
        $enrolinstances = enrol_get_instances($course->id, true);
        foreach ($enrolinstances as $courseenrolinstance) {
            if ($courseenrolinstance->enrol == "wordpress") {
                // Instance specified.
        ...
        foreach ($instances as $instance) {
            $enrolstatus = $enrol->can_wordpress_enrol($instance);
            if ($enrolstatus === true) {
                if ($instance->password and $params['password'] !== $instance->password) {
                    ...
                    if ($instance->customint1) {
                        ...
                        if (!enrol_wordpress_check_group_enrolment_key($course->id, $params['password'])) {
                            $warnings[] = array(
                                'item' => 'instance',
                                'itemid' => $instance->id,
                                'warningcode' => '2',
                                'message' => get_string('passwordinvalid', 'enrol_wordpress')
                            );
                            continue;
                        }
                    } else {
                        if ($enrol->get_config('showhint')) {
                            ...
                            $warnings[] = array(
                                ...
                                'itemid' => $instance->id,
                                'warningcode' => '3',
                                'message' => s(get_string('passwordinvalidhint', 'enrol_wordpress', $hint)) // message is PARAM_TEXT.
                            );
                            continue;
                        } else {
                            $warnings[] = array(
