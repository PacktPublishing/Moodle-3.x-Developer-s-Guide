...
/**
 * WordPress enrolment plugin tests.
 *
...
global $CFG;
require_once($CFG->dirroot.'/enrol/wordpress/lib.php');
require_once($CFG->dirroot.'/enrol/wordpress/locallib.php');

class enrol_wordpress_testcase extends advanced_testcase {

    public function test_basics() {
        $this->assertTrue(enrol_is_enabled('wordpress'));
        $plugin = enrol_get_plugin('wordpress');
        $this->assertInstanceOf('enrol_wordpress_plugin', $plugin);
        $this->assertEquals(1, get_config('enrol_wordpress', 'defaultenrol'));
        $this->assertEquals(ENROL_EXT_REMOVED_KEEP, get_config('enrol_wordpress', 'expiredaction'));
    }

    public function test_sync_nothing() {
        global $SITE;

        $wordpressplugin = enrol_get_plugin('wordpress');

        $trace = new null_progress_trace();

        // Just make sure the sync does not throw any errors when nothing to do.
        $wordpressplugin->sync($trace, null);
        $wordpressplugin->sync($trace, $SITE->id);
    }
    ...
    public function test_longtimnosee() {
        ...
        $this->assertEquals(3, $DB->count_records('enrol', array('enrol'=>'wordpress')));
        $instance1 = $DB->get_record('enrol', array('courseid'=>$course1->id, 'enrol'=>'wordpress'), '*', MUST_EXIST);
        $instance2 = $DB->get_record('enrol', array('courseid'=>$course2->id, 'enrol'=>'wordpress'), '*', MUST_EXIST);
        $instance3 = $DB->get_record('enrol', array('courseid'=>$course3->id, 'enrol'=>'wordpress'), '*', MUST_EXIST);
    ...
    public function test_expired() {
        ...
        $instance3 = $DB->get_record('enrol', array('courseid'=>$course3->id, 'enrol'=>'wordpress'), '*', MUST_EXIST);
        $this->assertEquals($studentrole->id, $instance3->roleid);
        $id = $wordpressplugin->add_instance($course3, array('status'=>ENROL_INSTANCE_ENABLED, 'roleid'=>$teacherrole->id));
        $instance3b = $DB->get_record('enrol', array('id'=>$id), '*', MUST_EXIST);
        ...
        $wordpressplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $wordpressplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $wordpressplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now-60);

        $wordpressplugin->enrol_user($instance3, $user1->id, $studentrole->id, 0, 0);
        $wordpressplugin->enrol_user($instance3, $user2->id, $studentrole->id, 0, $now-60*60);
        $wordpressplugin->enrol_user($instance3, $user3->id, $studentrole->id, 0, $now+60*60);
        $wordpressplugin->enrol_user($instance3b, $user1->id, $teacherrole->id, $now-60*60*24*7, $now-60);
        $wordpressplugin->enrol_user($instance3b, $user4->id, $teacherrole->id);
        ...
        $this->assertEquals(ENROL_EXT_REMOVED_KEEP, $wordpressplugin->get_config('expiredaction'));
        $wordpressplugin->sync($trace, null);
        $this->assertEquals(10, $DB->count_records('user_enrolments'));
        $this->assertEquals(10, $DB->count_records('role_assignments'));


        $wordpressplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $wordpressplugin->sync($trace, $course2->id);
        $this->assertEquals(10, $DB->count_records('user_enrolments'));
        $this->assertEquals(10, $DB->count_records('role_assignments'));
        ...
        $wordpressplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
    ...
    public function test_send_expiry_notifications() {
        ...
        /** @var $wordpressplugin enrol_wordpress_plugin */
        $wordpressplugin = enrol_get_plugin('wordpress');
        ...
        $this->assertEquals(4, $DB->count_records('enrol', array('enrol'=>'wordpress')));
        ...
        $instance4 = $DB->get_record('enrol', array('courseid'=>$course4->id, 'enrol'=>'wordpress'), '*', MUST_EXIST);
        ...
        $wordpressplugin->enrol_user($instance1, $user1->id, $studentrole->id, 0, $now + 60*60*24*1, ENROL_USER_SUSPENDED); // Suspended users are not notified.
        $wordpressplugin->enrol_user($instance1, $user2->id, $studentrole->id, 0, $now + 60*60*24*5);                       // Above threshold are not notified.
        $wordpressplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now + 60*60*24*3 + 60*60);               // Less than one day after threshold - should be notified.
        $wordpressplugin->enrol_user($instance1, $user4->id, $studentrole->id, 0, $now + 60*60*24*4 - 60*3);                // Less than one day after threshold - should be notified.
        $wordpressplugin->enrol_user($instance1, $user5->id, $studentrole->id, 0, $now + 60*60);                            // Should have been already notified.
        $wordpressplugin->enrol_user($instance1, $user6->id, $studentrole->id, 0, $now - 60);                               // Already expired.
        ...
        $wordpressplugin->enrol_user($instance2, $user1->id, $studentrole->id);
        $wordpressplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now + 60*60*24*1 + 60*3);                // Above threshold are not notified.
        $wordpressplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);               // Less than one day after threshold - should be notified.

        $manualplugin->enrol_user($maninstance3, $user1->id, $editingteacherrole->id);
        $wordpressplugin->enrol_user($instance3, $user2->id, $studentrole->id, 0, $now + 60*60*24*1 + 60);                  // Above threshold are not notified.
        $wordpressplugin->enrol_user($instance3, $user3->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);               // Less than one day after threshold - should be notified.

        $manualplugin->enrol_user($maninstance4, $user4->id, $editingteacherrole->id);
        $wordpressplugin->enrol_user($instance4, $user5->id, $studentrole->id, 0, $now + 60*60*24*1 + 60);
        $wordpressplugin->enrol_user($instance4, $user6->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);
        ...
        $wordpressplugin->send_expiry_notifications($trace);
        ...
        $wordpressplugin->set_config('expirynotifylast', time() - 60*60*24);
        $wordpressplugin->set_config('expirynotifyhour', '24');

        $wordpressplugin->send_expiry_notifications($trace);
        $this->assertEquals(0, $sink->count());

        $wordpressplugin->set_config('expirynotifyhour', '0');
        $wordpressplugin->send_expiry_notifications($trace);
    ...
    public function test_show_enrolme_link() {
        ...
        $DB->update_record('enrol', $instance1);
        $wordpressplugin->update_status($instance1, ENROL_INSTANCE_ENABLED);

        // New enrolments are not allowed, but enrolment instance is enabled.
        $instance2 = $DB->get_record('enrol', array('courseid'=>$course2->id, 'enrol'=>'wordpress'), '*', MUST_EXIST);
        $instance2->customint6 = 0;
        $DB->update_record('enrol', $instance2);
        $wordpressplugin->update_status($instance2, ENROL_INSTANCE_ENABLED);

        // New enrolments are allowed , but enrolment instance is disabled.
        $instance3 = $DB->get_record('enrol', array('courseid'=>$course3->id, 'enrol'=>'wordpress'), '*', MUST_EXIST);
        $instance3->customint6 = 1;
        $DB->update_record('enrol', $instance3);
        $wordpressplugin->update_status($instance3, ENROL_INSTANCE_DISABLED);

        // New enrolments are not allowed and enrolment instance is disabled.
        $instance4 = $DB->get_record('enrol', array('courseid'=>$course4->id, 'enrol'=>'wordpress'), '*', MUST_EXIST);
        $instance4->customint6 = 0;
        $DB->update_record('enrol', $instance4);
        $wordpressplugin->update_status($instance4, ENROL_INSTANCE_DISABLED);

        // Cohort member test.
        $instance5 = $DB->get_record('enrol', array('courseid'=>$course5->id, 'enrol'=>'wordpress'), '*', MUST_EXIST);
        $instance5->customint6 = 1;
        $instance5->customint5 = $cohort1->id;
        $DB->update_record('enrol', $instance1);
        $wordpressplugin->update_status($instance5, ENROL_INSTANCE_ENABLED);

        $id = $wordpressplugin->add_instance($course5, $selfplugin->get_instance_defaults());
        $instance6 = $DB->get_record('enrol', array('id'=>$id), '*', MUST_EXIST);
        $instance6->customint6 = 1;
        $instance6->customint5 = $cohort2->id;
        $DB->update_record('enrol', $instance1);
        $wordpressplugin->update_status($instance6, ENROL_INSTANCE_ENABLED);

        // Enrol start date is in future.
        $instance7 = $DB->get_record('enrol', array('courseid'=>$course6->id, 'enrol'=>'wordpress'), '*', MUST_EXIST);
        $instance7->customint6 = 1;
        $instance7->enrolstartdate = time() + 60;
        $DB->update_record('enrol', $instance7);
        $wordpressplugin->update_status($instance7, ENROL_INSTANCE_ENABLED);

        // Enrol start date is in past.
        $instance8 = $DB->get_record('enrol', array('courseid'=>$course7->id, 'enrol'=>'wordpress'), '*', MUST_EXIST);
        $instance8->customint6 = 1;
        $instance8->enrolstartdate = time() - 60;
        $DB->update_record('enrol', $instance8);
        $wordpressplugin->update_status($instance8, ENROL_INSTANCE_ENABLED);

        // Enrol end date is in future.
        $instance9 = $DB->get_record('enrol', array('courseid'=>$course8->id, 'enrol'=>'wordpress'), '*', MUST_EXIST);
        $instance9->customint6 = 1;
        $instance9->enrolenddate = time() + 60;
        $DB->update_record('enrol', $instance9);
        $wordpressplugin->update_status($instance9, ENROL_INSTANCE_ENABLED);

        // Enrol end date is in past.
        $instance10 = $DB->get_record('enrol', array('courseid'=>$course9->id, 'enrol'=>'wordpress'), '*', MUST_EXIST);
        $instance10->customint6 = 1;
        $instance10->enrolenddate = time() - 60;
        $DB->update_record('enrol', $instance10);
        $wordpressplugin->update_status($instance10, ENROL_INSTANCE_ENABLED);

        // Maximum enrolments reached.
        $instance11 = $DB->get_record('enrol', array('courseid'=>$course10->id, 'enrol'=>'wordpress'), '*', MUST_EXIST);
        $instance11->customint6 = 1;
        $instance11->customint3 = 1;
        $DB->update_record('enrol', $instance11);
        $wordpressplugin->update_status($instance11, ENROL_INSTANCE_ENABLED);
        $wordpressplugin->enrol_user($instance11, $user2->id, $studentrole->id);

        // Maximum enrolments not reached.
        $instance12 = $DB->get_record('enrol', array('courseid'=>$course11->id, 'enrol'=>'wordpress'), '*', MUST_EXIST);
        $instance12->customint6 = 1;
        $instance12->customint3 = 1;
        $DB->update_record('enrol', $instance12);
        $wordpressplugin->update_status($instance12, ENROL_INSTANCE_ENABLED);
     ...
     */
    public function test_can_wordpress_enrol() {
        ...
        $expectederrorstring = get_string('canntenrol', 'enrol_wordpress');
        ...
        $DB->update_record('enrol', $instance1);
        $wordpressplugin->update_status($instance1, ENROL_INSTANCE_ENABLED);
        $wordpressplugin->enrol_user($instance1, $user2->id, $editingteacherrole->id);

        $this->setUser($guest);
        $noaccesshtml = get_string('noguestaccess', 'enrol') . $OUTPUT->continue_button(get_login_url());
        $this->assertSame($noaccesshtml, $wordpressplugin->can_wordpress_enrol($instance1, true));

        $this->setUser($user1);
        $this->assertTrue($selfplugin->can_wordpress_enrol($instance1, true));

        // Active enroled user.
        $this->setUser($user2);
        $wordpressplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $this->setUser($user1);
        $this->assertSame($expectederrorstring, $wordpressplugin->can_wordpress_enrol($instance1, true));
    }

    /**
     * Test enrol_wordpress_check_group_enrolment_key
     */
    public function test_enrol_wordpress_check_group_enrolment_key() {
        global $DB;
        ...
        $result = enrol_wordpress_check_group_enrolment_key($course->id, 'invalidpassword');
        $this->assertFalse($result);

        $result = enrol_wordpress_check_group_enrolment_key($course->id, 'thepassword');
        ...
        $result = enrol_wordpress_check_group_enrolment_key($othercourse->id, 'thepassword');
        $this->assertFalse($result);
    }
}
