...
/**
 * Wordpress enrol external PHPunit tests
 *
...
require_once($CFG->dirroot . '/enrol/wordpress/externallib.php');

class enrol_wordpress_external_testcase extends externallib_advanced_testcase {
    ...
    public function test_get_instance_info() {
        ...
        // Check if wordpress enrolment plugin is enabled.
        $wordpressplugin = enrol_get_plugin('wordpress');
        $this->assertNotEmpty($wordpressplugin);
        ...
        $instanceid1 = $wordpressplugin->add_instance($course, array('status' => ENROL_INSTANCE_ENABLED,
                                                                'name' => 'Test instance 1',
                                                                'customint6' => 1,
                                                                'roleid' => $studentrole->id));
        $instanceid2 = $wordpressplugin->add_instance($course, array('status' => ENROL_INSTANCE_DISABLED,
                                                                'customint6' => 1,
                                                                'name' => 'Test instance 2',
                                                                'roleid' => $studentrole->id));

        $instanceid3 = $wordpressplugin->add_instance($course, array('status' => ENROL_INSTANCE_ENABLED,
                                                                'roleid' => $studentrole->id,
                                                                'customint6' => 1,
        ...
        $this->setAdminUser();
        $instanceinfo1 = enrol_wordpress_external::get_instance_info($instanceid1);
        $instanceinfo1 = external_api::clean_returnvalue(enrol_wordpress_external::get_instance_info_returns(), $instanceinfo1);
