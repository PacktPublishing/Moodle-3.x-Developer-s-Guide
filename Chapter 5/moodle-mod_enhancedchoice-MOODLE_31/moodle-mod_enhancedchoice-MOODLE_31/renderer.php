...
class mod_enhancedchoice_renderer extends plugin_renderer_base {
        ...
        if (!empty($options['hascapability']) && ($options['hascapability'])) {
            if ($availableoption < 1) {
               $html .= html_writer::tag('label', get_string('choicefull', 'choice'));
                ...
                $html .= html_writer::empty_tag('input', array('type'=>'submit', 'value'=>get_string('savemychoice','enhancedchoice'), 'class'=>'button'));
            }

            if (!empty($options['allowupdate']) && ($options['allowupdate'])) {
                $url = new moodle_url('view.php', array('id'=>$coursemoduleid, 'action'=>'delchoice', 'sesskey'=>sesskey()));
                $html .= html_writer::link($url, get_string('removemychoice','enhancedchoice'));
            }
        } else {
    ...
    public function display_publish_name_vertical($choices) {
        global $PAGE;
        $html ='';
        $html .= html_writer::tag('h2',format_string(get_string("responses", "enhancedchoice")), array('class'=>'main'));
        ...
        $table->summary = get_string('responsesto', 'enhancedchoice', format_string($choices->name));
        $table->data = array();
        ...
        if ($choices->viewresponsecapability && $choices->deleterepsonsecapability) {
            ...
            $actiondata .= html_writer::tag('label', ' ' . get_string('withselected', 'enhancedchoice') . ' ', array('for'=>'menuaction'));

            $actionurl = new moodle_url($PAGE->url, array('sesskey'=>sesskey(), 'action'=>'delete_confirmation()'));
            $select = new single_select($actionurl, 'action', array('delete'=>get_string('delete')), null, array(''=>get_string('chooseaction', 'enhancedchoice')), 'attemptsform');
    ...
    public function display_publish_anonymous_vertical($choices) {
        ...
        $tableheader->text = html_writer::tag('div', get_string('choiceoptions', 'enhancedchoice'), array('class' => 'accesshide'));
        $rows['header'][] = $tableheader;

        // graph row header
        $graphheader = clone($headercelldefault);
        $graphheader->text = html_writer::tag('div', get_string('responsesresultgraphheader', 'enhancedchoice'), array('class' => 'accesshide'));
        ...
        $userpercentageheader->text = get_string('numberofuserinpercentage', 'enhancedchoice');
        ...
        foreach ($choices->options as $optionid => $option) {
            ...
            $displaygraph = html_writer::tag('img','', array('style'=>'height:'.$height.'px;width:49px;', 'alt'=>'', 'src'=>$this->output->pix_url('column', 'enhancedchoice')));
            ...
            $usernumber .= html_writer::tag('div', ' '.$numberofuser.'', array('class'=>'numberofuser', 'title'=> get_string('numberofuser', 'enhancedchoice')));
        ...
        $header = html_writer::tag('h2',format_string(get_string("responses", "enhancedchoice")));
        $html .= html_writer::tag('div', $header, array('class'=>'responseheader'));
        $html .= html_writer::tag('a', get_string('skipresultgraph', 'enhancedchoice'), array('href'=>'#skipresultgraph', 'class'=>'skip-block'));
        $html .= html_writer::tag('div', html_writer::table($table), array('class'=>'response'));
    ...
    public function display_publish_anonymous_horizontal($choices) {
        ...
        $tableheadertext->text = get_string('choiceoptions', 'enhancedchoice');

        $tableheadernumber = clone($columnheaderdefault);
        $tableheadernumber->text = get_string('numberofuser', 'enhancedchoice');

        $tableheaderpercentage = clone($columnheaderdefault);
        $tableheaderpercentage->text = get_string('numberofuserinpercentage', 'enhancedchoice');

        $tableheadergraph = clone($columnheaderdefault);
        $tableheadergraph->text = get_string('responsesresultgraphheader', 'enhancedchoice');
        ...
        foreach ($choices->options as $optionid => $options) {
            ...
            $attributes['alt'] = '';
            $attributes['src'] = $this->output->pix_url('row', 'enhancedchoice');
